(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('tdxmainApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-tdxmainApp-error');
                var entityKey = httpResponse.headers('X-tdxmainApp-params');
                if (errorHeader) {
